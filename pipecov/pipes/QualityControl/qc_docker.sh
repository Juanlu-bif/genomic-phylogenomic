...
while getopts "i:1:2:q:l:o:t:m:" opt; do
	case $opt in
		i) SEQUENCER="$OPTARG"
		;;
...
then
	...
	COMMON_PATH=$({ echo $FULL_PATH_FOR; echo $FULL_PATH_REV;} | sed -e 'N;s/^\(.*\).*\n\1.*$/\1\n\1/;D')
